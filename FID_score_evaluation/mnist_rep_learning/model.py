...
@torch.no_grad()
def sample_IADB(D,VAE,  period, latent_dims=2, device='cuda'):
    # starting points x_alpha = x_0
    batchsize = 64
    torch.manual_seed(0)
    x_0 = torch.randn(batchsize, 1, 32, 32, device=device)
    x_alpha = x_0
    embedding = torch.randn(batchsize, latent_dims, device=device)
    ...
    for t in tqdm(range(T), "sampling loop"):
        ...
        # current alpha value
        alpha = t / T * torch.ones(batchsize, device=device)
    ...
    saveImage('/home/rajan/Desktop/rajan_thesis/thesis_work/FID_score_evaluation/mnist_rep_learning/samples/generated_mnist_'+str(t)+'_'+str(period)+'_.png', result)
    return x_alpha
