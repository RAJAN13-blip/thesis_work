...
def generate_data():
    ...
    for i in range(10):
        p0_data_list.append(scale_data(make_swirl_data(N=2195)))
        p1_data_list.append(scale_data(make_2d_array(*get_masked_data('S', intensity=25, rand=True))))
...
def create_images(prob_dist ,type):
    ...
    for i in range(len(prob_dist)):
        ...
        plt.axis('off')
        plt.savefig(f'{type}_images/{type}_image_{i}.png', transparent=True)
        plt.close()
...
class PROB(Dataset):
    def __init__(self, two_dim_array_p0: np.array, two_dim_array_p1: np.array):
        self.prob_x0 = two_dim_array_p0[:,0]
        self.prob_y0 = two_dim_array_p0[:,1]
  
        self.prob_x1 = two_dim_array_p1[:,0]+2
        self.prob_y1 = two_dim_array_p1[:,1]
        self.length = two_dim_array_p0.shape[0]
    ...
    def __getitem__(self, index):
        # index1 = np.random.randint(index,self.length-1)
        # index2 = np.random.randint(index,self.length-1)
        x_0,y_0 = self.prob_x0[index], self.prob_y0[index]
        x_1,y_1 = self.prob_x1[index], self.prob_y1[index]
        a0 = torch.tensor([x_0, y_0],dtype=torch.float32)
        a1 = torch.tensor([x_1, y_1],dtype=torch.float32)
        return a0, a1
